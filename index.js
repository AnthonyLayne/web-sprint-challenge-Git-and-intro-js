...
function get20s(array, name) {
  ...
  for (let i = 0; i < array.length; i++) {
    const artist = array[i]; // { id: 0, name: "Amedeo Modigliani", years: "1884 - 1920", ...}
    const years = artist.years.split(" - "); // ["1884", "1920"]
    const born = Number(years[0]); // 1884
    const died = Number(years[1]); // 1920
...
function addArtist(arr) {
  const newArt = {
    ...
    genre: "Web Design",
    nationality: "USA",
    bio: "Currenty attending bloomTech.",
  };
  arr.push(newArt);
  return arr;
}
